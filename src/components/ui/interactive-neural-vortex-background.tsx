 ...
 *
 * Uses plain WebGL1 (no Three.js) to render a full-viewport animated
 * neural-lattice shader that reacts to pointer position and scroll.
 * Self-contained: mounts a fixed canvas at `-z-10`, respects
 * `prefers-reduced-motion`, and cleans up its RAF loop on unmount.
 *
 * Original shader: community component adapted for our shadcn/Tailwind
 * setup. Removed the demo hero + global style injection from the source.
 */
export function InteractiveNeuralVortex({ className, ...props }: Props) {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      // WebGL unavailable — fail silently (we're just a background).
      return;
    }
    ...
    if (!prefersReduced) render();
      ...
      // One frame so the static pattern appears.
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    }
  ...
  return (
    <canvas
      ...
      aria-hidden="true"
      className={cn(
        'pointer-events-none fixed inset-0 w-full h-full -z-10 opacity-70',
        className,
      )}
      {...props}
    />
